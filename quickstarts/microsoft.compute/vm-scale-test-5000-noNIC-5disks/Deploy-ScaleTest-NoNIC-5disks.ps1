<#
.SYNOPSIS
    Deploys 5000 VMs with 3 data disks each using inline NIC configuration (no separate NIC resources).

.DESCRIPTION
    Scale test: 5,000 VMs × 4 disks (1 OS + 3 PremiumV2_LRS data) = 20,000 total disks.
    Uses networkInterfaceConfigurations to eliminate ARM NIC PUT throttling.
...
.EXAMPLE
    .\Deploy-ScaleTest-NoNIC-5disks.ps1 -SkipWait -SkipCleanup
    Deploys 5000 VMs with 5 data disks each, skips wait and cleanup.
#>
...
$VmBatchTemplatePath = Join-Path $ScriptDir "vm-noNIC-5disks-batch-deploy.json"
...
Write-Info "Approach: NoNIC + 3 DataDisks (inline NIC, 3x PremiumV2_LRS per VM)"
Write-Info "Total disks: $($TotalVmCount * 4) (1 OS + 3 data per VM)"
...
try {
    $usage = Get-AzVMUsage -Location $Location | Where-Object {
        $_.Name.Value -like "*StandardDDSv5Family*"
    }
    if ($usage) {
        foreach ($u in $usage) {
    ...
    if ($_.Exception.Message -eq "Insufficient DDSv5 vCPU quota.") { throw }
...
Write-Info "Disks per VM: 4 (1 OS StandardSSD_LRS + 3 data PremiumV2_LRS)"
Write-Info "Total Disks: $($TotalVmCount * 4) | Total Data Disks: $($TotalVmCount * 3)"
...
Write-Phase "PHASE 3" "Launching $batchCount parallel batch deployments (5 data disks per VM)..."
...
for ($i = 0; $i -lt $batchCount; $i++) {
    ...
    $dn = "batch-{0:D2}" -f $i
    Write-Info "Queuing ${dn}: VMs ${si}-$($si + $cnt - 1), Zone $z, $($cnt * 3) data disks"
...
Write-Info "Disks per VM:      4 (1 OS + 3 data)"
Write-Info "Total Disks:       $($TotalVmCount * 4)"
...
Write-Host ""
Write-Host "=" * 60 -ForegroundColor White
Write-Phase "DONE" "Scale test complete (NoNIC + 3 DataDisks)"
Write-Host "=" * 60 -ForegroundColor White
